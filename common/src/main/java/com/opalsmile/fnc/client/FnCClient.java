...
public class FnCClient {
    ...
    public static final KeyMapping ANTLER_KEYBIND = new KeyMapping("key.featuresandcreatures.antler_headdress",
            InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_RIGHT_SHIFT, "key.categories.gameplay");
    ...
    public static void registerItemProperties() {
        ItemProperties.register(FnCItems.DOWSING_ROD.get(), FnCConstants.resourceLocation("dowsing"), (itemStack, clientLevel,
                                                                                                       livingEntity, seed) -> {

            if (!(livingEntity instanceof Player player)) return 0.0f;

            //if selected and second hand is empty.
            if (player.getItemBySlot(EquipmentSlot.MAINHAND) != itemStack || !player.getItemBySlot(EquipmentSlot.OFFHAND).isEmpty()) {
                return 0.0f;
            }
